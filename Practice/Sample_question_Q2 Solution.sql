...
SELECT distinct patient.name FROM patient JOIN APPOINTMENTS on patient.patient_id=appointments.patient_id
JOIN DOCTOR on doctor.doctor_id = appointments.doctor_id
...
WITH num (patient_id,value) as (SELECT patient_id, count (*) FROM APPOINTMENTS group by patient_id) SELECT name
FROM PATIENT join num using(patient_id) 
WHERE value>1;

SELECT name
FROM PATIENT
WHERE patient_id in (SELECT patient_id FROM  Appointments  group by patient_id having count(doctor_id)>1);
...
-- 3.1
SELECT p.name ,p.patient_id
FROM PATIENT p
LEFT OUTER JOIN APPOINTMENTS a
    ON p.patient_id = a.patient_id
WHERE a.doctor_id IS NULL;

(SELECT name FROM patient) minus (SELECT name
...
SELECT d.name
FROM DOCTOR d, APPOINTMENT_COUNT c, AVG_APPOINTMENT 
WHERE d.doctor_id = c.doctor_id
    AND c.app_count > AVG_APPOINTMENT.value;
...
(SELECT doctor.name FROM doctor join  appointments using (doctor_id) 
join patient using (patient_id)
WHERE patient.gender='Male') intersect 
  (SELECT doctor.name FROM doctor join  appointments using (doctor_id) 
join patient using (patient_id)
